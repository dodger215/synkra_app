<x-layouts.guest title="Login | {{ $store->store_name }}">
        ...
        <div class="max-w-md w-full space-y-8 bg-white p-10 rounded-2xl shadow-xl">
            <div>
                <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900">
                    Welcome back to {{ $store->store_name }}
                </h2>
                <p class="mt-2 text-center text-sm text-gray-600">
                    Or
                    <a href="{{ route('storefront.register', $store->id) }}" class="font-medium text-orange-600 hover:text-orange-500">
                        create a new account
                    </a>
                </p>
            </div>
            <form class="mt-8 space-y-6" action="{{ route('storefront.login', $store->id) }}" method="POST">
                @csrf

<x-layouts.app title="User Management">
    ...
    <div class="synkra-dashboard-container" style="padding: 2rem; max-width: 1000px; margin: 0 auto;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 2rem;">
            ...
            <button onclick="openSynkraModal('inviteUserModal')" class="synkra-btn synkra-btn-primary" style="background: var(--primary); border: none; color: white; padding: 0.6rem 1.25rem; border-radius: 8px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 8px; transition: all 0.2s;">
        ...
        <div class="synkra-card" style="background: var(--surface); border-radius: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.02); border: 1px solid var(--border); overflow: hidden;">
            ...
            @else
                <div class="synkra-table-container">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="text-align: right;">
                                    <div class="synkra-table-actions" style="justify-content: flex-end;">
                                        <form action="{{ route('invite.resend', $invite->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="synkra-table-action-btn" style="color: var(--primary);" title="Resend Invite">
                                        ...
                                        <form action="{{ route('invite.revoke', $invite->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to revoke this invitation?');">
                                            @csrf
                                            <button type="submit" class="synkra-table-action-btn" style="color: var(--danger);" title="Revoke Invite">
    ...
    <x-ui.modal id="inviteUserModal" triggerId="inviteUserModal-trigger-btn" title="Invite Team Member">
        ...
        <x-slot:footer>
            <button type="button" class="synkra-btn synkra-btn-secondary" style="background: transparent; border: 1px solid var(--border); color: var(--text-secondary); cursor: pointer; padding: 0.5rem 1rem; border-radius: 8px;" onclick="closeSynkraModal('inviteUserModal')">Cancel</button>
            <button type="button" class="synkra-btn synkra-btn-primary" style="background: var(--primary); border: none; color: white; cursor: pointer; padding: 0.5rem 1rem; border-radius: 8px; font-weight: 600; display: flex; align-items: center; gap: 6px;" onclick="document.getElementById('inviteUserForm').submit()">

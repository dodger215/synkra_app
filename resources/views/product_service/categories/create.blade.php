    ...
    <x-ui.grid>
        <div class="flowexa-dashboard-container" style="padding:2rem;max-width:700px;margin:0 auto;">
            <div style="margin-bottom:1.5rem;">
                ...
                @endif
                <h1 style="color:var(--headings);margin:0 0 .25rem 0;">Create Category</h1>
                <p style="color:var(--text-secondary);margin:0;">
                    @if($fromProduct)
                        Add a category and return to your product draft.
                    @else
            ...
            <div class="flowexa-card" style="background:var(--surface);border-radius:16px;padding:1.5rem;border:1px solid var(--border);">
                <form action="{{ route('product_service.categories.store') }}" method="POST">
                    ...
                    <div style="display:flex;flex-direction:column;gap:1.1rem;">
                        <x-ui.input name="name" label="Category Name" value="{{ old('name') }}" placeholder="e.g. Electronics" required />
                        <div>
                            <label style="font-size:.85rem;font-weight:600;color:var(--headings);display:block;margin-bottom:.4rem;">Description</label>
                            <textarea name="description" rows="4" style="width:100%;padding:.7rem 1rem;background:var(--surface-secondary);border:1px solid var(--border);border-radius:8px;color:var(--text-primary);font-size:.95rem;font-family:inherit;outline:none;">{{ old('description') }}</textarea>
                        </div>
                    </div>
                    ...
                    <div style="margin-top:1.5rem;display:flex;justify-content:flex-end;gap:1rem;border-top:1px solid var(--border);padding-top:1.5rem;">
                        @if($fromProduct)
                            <a href="{{ route('product_service.products.create') }}" style="background:transparent;border:1px solid var(--border);color:var(--text-primary);padding:.75rem 1.5rem;border-radius:8px;font-weight:600;text-decoration:none;">Cancel</a>
                        @else
                            <a href="{{ route('product_service.categories.index') }}" style="background:transparent;border:1px solid var(--border);color:var(--text-primary);padding:.75rem 1.5rem;border-radius:8px;font-weight:600;text-decoration:none;">Cancel</a>
                        @endif

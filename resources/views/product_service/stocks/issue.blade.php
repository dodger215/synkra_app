<x-layouts.app title="Issue Stock">
    <x-ui.grid>
        <div class="flowexa-dashboard-container" style="padding:2rem;max-width:720px;margin:0 auto;">
            ...
            <h1 style="color:var(--headings);margin:0 0 .4rem 0;">Issue Stock</h1>
            <p style="color:var(--text-secondary);margin:0 0 1.5rem 0;">Remove stock from a location.</p>
            ...
            <div class="flowexa-card" style="background:var(--surface);border-radius:16px;padding:1.5rem;border:1px solid var(--border);">
                <form action="{{ route('product_service.stocks.issue.store') }}" method="POST">
                    @csrf
                    @include('product_service.stocks.partials.stock-form-fields', ['products' => $products, 'locations' => $locations, 'formType' => 'issue'])
                    <div style="margin-top:1.5rem;display:flex;justify-content:flex-end;gap:.75rem;">
                        ...
                        <button type="submit" style="background:var(--primary);border:none;color:white;padding:.65rem 1.5rem;border-radius:8px;font-weight:600;cursor:pointer;">Issue Stock</button>
                    </div>
                </form>
            </div>
        </div>
